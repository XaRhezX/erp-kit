...
return new class extends Migration
{
    ...
    public function up()
    {
        Schema::create('location_sub_districts', function (Blueprint $table) {
            $table->id();
            $table->string('name');
            $table->string('postal_code');
            $table->foreignId('province_id')->constrained('location_provinces');
        ...
        //Insert Data
        $path = database_path('csv/villages.csv');
        $data = (new FastExcel())->import($path);
        $data->map(function (array $row) {
            return Arr::only($row, ["id", "name", "postal_code", "province_id", "city_id", "district_id", "latitude", "longitude"]);
        })->chunk(1000)->each(function (Collection $chunk) {
            LocationSubDistrict::upsert(
    ...
    {
        Schema::dropIfExists('location_sub_districts');
    }
};
